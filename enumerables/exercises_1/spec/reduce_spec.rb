...
RSpec.describe 'reduce' do
  ...
  it 'capitalize key words in phrase' do
    keywords = ["fish", "blue"]
    result = keywords.reduce('one fish two fish red fish blue fish') do |upcase, word|
      upcase.gsub(word, word.upcase)
    end
  ...
  it 'divides 560 by a bunch of numbers' do
    numbers = [2, 2, 2, 5, 7]
    result = numbers.reduce(560) do |quiotent, number|
      quiotent / number
    end
  ...
  it 'subtract smallest values from 100' do
    ...
    result = elements.reduce(100) do |num, array|
      num -= array.min
    end
  ...
  it 'adds all second values together' do
    ...
    result = elements.reduce(0) do |num, array|
      num += array[1]
    end
